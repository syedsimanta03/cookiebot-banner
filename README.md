<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Cookiebot Banner</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link
      href="https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900&display=swap"
      rel="stylesheet"
    />
    <style>
      /* Custom transitions for accordion animations */
      *,
      *::before,
      *::after {
        margin: 0;
        padding: 0;
        box-sizing: border-box;
      }
      html {
        scroll-behavior: smooth;
      }
      body {
        padding: 0;
        margin: 0;
        font-family: 'Inter', sans-serif;
        font-size: 1rem;
        font-weight: <weight>;
        font-style: normal;
      }
      .accordion-content {
        max-height: 0;
        overflow: hidden;
        transition: max-height 0.3s ease-out;
      }
      .accordion-content.open {
        max-height: 1000px;
        transition: max-height 0.5s ease-in;
      }
    </style>
  </head>
  <body class="bg-gray-100 min-h-screen flex items-center justify-center p-4">
    <!-- Cookiebot Banner Container -->
    <div
      class="cookiebot-banner bg-white shadow-lg rounded-lg max-w-3xl w-full mx-auto overflow-hidden"
    >
      <!-- Banner Header -->
      <div class="p-4 sm:p-6 border-b border-gray-200">
        <div class="flex justify-between items-center">
          <img
            class="w-6 h-auto"
            src="data:image/png;base64,"
            alt="logo"
          />
          <!-- <button class="text-gray-500 hover:" id="close-banner">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-6 w-6"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M6 18L18 6M6 6l12 12"
              />
            </svg>
          </button> -->
          <a
            class="w-full"
            href="https://www.cookiebot.com/en/what-is-behind-powered-by-cookiebot/"
            rel="noopener nofollow"
            target="_blank"
            id="CybotCookiebotDialogPoweredbyCybot"
            aria-label="Cookiebot - opens in a new window"
            ><svg
              class="w-[100px] h-auto ml-auto"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 458 121"
            >
              <path
                d="M83.8673 1.30835V46.7911C83.8673 50.2873 84.9356 52.1186 87.5732 52.1186C90.3443 52.1186 91.2124 49.9876 91.2124 46.7911V1.30835H102.864V46.0586C102.864 56.6469 97.9899 61.8744 87.4063 61.8744C76.8226 61.8744 72.3154 56.8133 72.3154 45.9587V1.30835H83.9006H83.8673Z"
              ></path>
              <path
                d="M119.057 42.5959V46.7247C119.057 50.6536 120.025 52.4849 122.663 52.4849C125.301 52.4849 125.935 49.7879 125.935 46.9577C125.935 41.3307 124.867 39.6991 118.757 34.5715C112.013 28.8778 108.54 25.5149 108.54 16.6914C108.54 7.86784 111.378 0.30957 122.73 0.30957C134.782 0.30957 136.552 8.50047 136.552 15.3262V18.7557H125.668V15.193C125.668 11.5637 125.067 9.76573 122.797 9.76573C120.66 9.76573 119.992 11.597 119.992 14.96C119.992 18.556 120.66 20.3207 125.601 24.183C134.415 31.0754 137.553 34.9711 137.553 44.7935C137.553 54.6159 134.348 61.8412 122.463 61.8412C110.577 61.8412 107.772 54.9822 107.772 46.1253V42.5293H119.057V42.5959Z"
              ></path>
              <path
                d="M166.466 34.638H154.113V50.9199H168.57L167.201 61.0087H142.595V1.30835H167.134V11.4637H154.147V24.5492H166.5V34.638H166.466Z"
              ></path>
              <path
                d="M184.195 35.0708V61.042H172.71V1.30835H187.667C197.717 1.30835 202.658 5.70347 202.658 16.1585V18.3228C202.658 26.8466 199.252 29.3772 196.682 30.5758C200.421 32.3738 202.157 35.1041 202.157 43.4282C202.157 49.1885 202.057 57.9121 202.558 61.042H191.407C190.672 58.345 190.705 50.5536 190.705 42.9288C190.705 36.2029 189.938 35.0708 185.564 35.0708H184.195ZM184.228 26.0142H185.664C189.504 26.0142 191.073 24.8156 191.073 19.2551V16.3583C191.073 12.3627 190.271 10.365 186.031 10.365H184.228V26.0142Z"
              ></path>
              <path
                d="M237.513 41.5304V44.7601C237.513 52.2851 236.111 61.9078 222.389 61.9078C212.239 61.9078 207.832 56.5137 207.832 45.526V16.2585C207.832 5.87002 213.04 0.409424 222.622 0.409424C235.276 0.409424 237.313 8.70021 237.313 16.5249V20.2873H225.727V15.193C225.727 11.8967 225.026 10.2318 222.622 10.2318C220.219 10.2318 219.484 11.7968 219.484 15.193V46.7912C219.484 49.9877 220.018 52.1519 222.622 52.1519C225.227 52.1519 225.861 50.3206 225.861 46.5248V41.5637H237.513V41.5304Z"
              ></path>
              <path
                d="M266.326 34.638H253.973V50.9199H268.429L267.06 61.0087H242.454V1.30835H266.993V11.4637H254.006V24.5492H266.359V34.638H266.326Z"
              ></path>
              <path
                d="M272.569 61.0087V1.30835H285.323C287.226 8.60025 293.236 35.6369 293.703 38.2673H293.97C293.336 30.3095 292.935 19.4549 292.935 11.1308V1.30835H303.619V61.042H290.765C289.53 55.3483 282.986 24.4493 282.585 22.5514H282.285C282.719 29.5769 283.086 41.4637 283.086 50.8533V61.042H272.603L272.569 61.0087Z"
              ></path>
              <path
                d="M316.34 11.4637H307.626V1.30835H336.639V11.4637H327.858V61.042H316.373V11.4637H316.34Z"
              ></path>
              <path
                d="M352.197 35.0708V61.042H340.712V1.30835H355.669C365.719 1.30835 370.66 5.70347 370.66 16.1585V18.3228C370.66 26.8466 367.254 29.3772 364.684 30.5758C368.423 32.3738 370.159 35.1041 370.159 43.4282C370.159 49.1885 370.059 57.9121 370.56 61.042H359.409C358.674 58.345 358.707 50.5536 358.707 42.9288C358.707 36.2029 357.939 35.0708 353.566 35.0708H352.197ZM352.23 26.0142H353.666C357.505 26.0142 359.075 24.8156 359.075 19.2551V16.3583C359.075 12.3627 358.273 10.365 354.033 10.365H352.23V26.0142Z"
              ></path>
              <path d="M388.388 1.30835V61.042H376.903V1.30835H388.388Z"></path>
              <path
                d="M424.346 41.5304V44.7601C424.346 52.2851 422.944 61.9078 409.222 61.9078C399.072 61.9078 394.665 56.5137 394.665 45.526V16.2585C394.665 5.87002 399.873 0.409424 409.455 0.409424C422.109 0.409424 424.146 8.70021 424.146 16.5249V20.2873H412.56V15.193C412.56 11.8967 411.859 10.2318 409.455 10.2318C407.052 10.2318 406.317 11.7968 406.317 15.193V46.7912C406.317 49.9877 406.851 52.1519 409.455 52.1519C412.06 52.1519 412.694 50.3206 412.694 46.5248V41.5637H424.346V41.5304Z"
              ></path>
              <path
                d="M439.504 42.5959V46.7247C439.504 50.6536 440.472 52.4849 443.109 52.4849C445.747 52.4849 446.381 49.7879 446.381 46.9577C446.381 41.3307 445.313 39.6991 439.203 34.5715C432.459 28.8778 428.987 25.5149 428.987 16.6914C428.987 7.86784 431.825 0.30957 443.176 0.30957C455.229 0.30957 456.998 8.50047 456.998 15.3262V18.7557H446.114V15.193C446.114 11.5637 445.513 9.76573 443.243 9.76573C441.106 9.76573 440.438 11.597 440.438 14.96C440.438 18.556 441.106 20.3207 446.047 24.183C454.861 31.0754 458 34.9711 458 44.7935C458 54.6159 454.795 61.8412 442.909 61.8412C431.023 61.8412 428.219 54.9822 428.219 46.1253V42.5293H439.504V42.5959Z"
              ></path>
              <path
                d="M0 1.27515V31.5415C0 48.223 13.6218 61.8079 30.3486 61.8079C47.0754 61.8079 60.6972 48.223 60.6972 31.5415V1.27515H0ZM51.7829 31.5415C51.7829 43.3284 42.1675 52.9178 30.3486 52.9178C18.5297 52.9178 8.91427 43.3284 8.91427 31.5415V10.1653H51.8163V31.5415H51.7829Z"
              ></path>
              <path
                d="M24.1383 42.4295H24.1717H34.4548L34.4214 42.3962L46.2403 17.5571H35.9572L28.8124 32.5738L25.4404 26.5804H15.1572L24.1717 42.3962L24.1383 42.4295Z"
              ></path>
              <path
                d="M25.1735 105.926H30.6823C30.5488 108.922 29.9478 111.486 28.8794 113.65C27.811 115.815 26.2752 117.479 24.2387 118.611C22.2021 119.777 19.6647 120.343 16.6265 120.343C14.4563 120.343 12.4865 119.91 10.717 119.044C8.9475 118.179 7.44509 116.947 6.20978 115.348C4.97447 113.75 4.00625 111.819 3.33852 109.555C2.67078 107.291 2.33691 104.76 2.33691 101.963V93.9056C2.33691 91.1087 2.67078 88.6115 3.33852 86.3473C4.00625 84.0832 4.97447 82.152 6.24317 80.5538C7.51187 78.9556 9.04766 77.7236 10.8839 76.8579C12.7202 75.9922 14.7902 75.5593 17.0939 75.5593C19.9318 75.5593 22.3356 76.1254 24.3054 77.2574C26.2752 78.3895 27.7777 80.021 28.846 82.1853C29.9144 84.3496 30.5154 86.98 30.6155 90.1098H25.1067C24.9732 87.8124 24.6059 85.9478 24.0383 84.516C23.4708 83.0843 22.6027 82.0188 21.4675 81.3196C20.3324 80.6204 18.8634 80.2874 17.0605 80.2874C15.4579 80.2874 14.0891 80.6204 12.9205 81.253C11.752 81.8856 10.7838 82.8179 10.0493 83.9833C9.31475 85.1487 8.74718 86.5804 8.37992 88.2452C8.01267 89.91 7.81235 91.7746 7.81235 93.8057V101.897C7.81235 103.795 7.97928 105.593 8.27976 107.257C8.58024 108.922 9.08105 110.354 9.78217 111.619C10.4833 112.851 11.3847 113.85 12.4865 114.549C13.6216 115.249 14.9571 115.582 16.5597 115.582C18.5629 115.582 20.1655 115.249 21.3674 114.583C22.5359 113.917 23.404 112.885 24.0049 111.453C24.6059 110.021 24.9398 108.156 25.1401 105.826L25.1735 105.926Z"
              ></path>
              <path
                d="M36.291 105.126V102.063C36.291 99.566 36.6249 97.3351 37.2592 95.4372C37.8936 93.5393 38.795 91.9411 39.9302 90.6759C41.0653 89.4106 42.4008 88.445 43.9032 87.7791C45.4056 87.1131 47.0416 86.8135 48.7777 86.8135C50.5138 86.8135 52.2165 87.1464 53.7189 87.7791C55.2213 88.4117 56.5568 89.3773 57.7253 90.6759C58.8605 91.9411 59.7619 93.5393 60.3963 95.4372C61.0306 97.3351 61.3645 99.5327 61.3645 102.063V105.126C61.3645 107.624 61.0306 109.855 60.3963 111.752C59.7619 113.65 58.8605 115.249 57.7253 116.514C56.5902 117.779 55.2547 118.745 53.7523 119.377C52.2499 120.01 50.5806 120.343 48.8445 120.343C47.1083 120.343 45.439 120.01 43.9366 119.377C42.4342 118.745 41.0987 117.779 39.9302 116.514C38.795 115.249 37.8936 113.65 37.2592 111.752C36.6249 109.855 36.291 107.657 36.291 105.126ZM41.5995 102.063V105.126C41.5995 106.891 41.7664 108.456 42.1337 109.788C42.501 111.12 43.0018 112.252 43.6695 113.151C44.3372 114.05 45.1051 114.716 45.9732 115.182C46.8412 115.615 47.8095 115.848 48.8445 115.848C50.0464 115.848 51.0814 115.615 52.0162 115.182C52.9176 114.749 53.6855 114.05 54.2865 113.151C54.8875 112.252 55.3215 111.153 55.622 109.788C55.9224 108.456 56.056 106.891 56.056 105.126V102.063C56.056 100.298 55.8891 98.7336 55.5218 97.4017C55.1546 96.0699 54.6538 94.9711 53.986 94.0388C53.3183 93.1398 52.5504 92.4406 51.6489 92.0077C50.7475 91.5749 49.7793 91.3418 48.7443 91.3418C47.7093 91.3418 46.7745 91.5749 45.9064 92.0077C45.0384 92.4406 44.2705 93.1398 43.6027 94.0388C42.9684 94.9378 42.4342 96.0699 42.1003 97.4017C41.7331 98.7336 41.5661 100.265 41.5661 102.063H41.5995Z"
              ></path>
              <path
                d="M67.1074 105.126V102.063C67.1074 99.566 67.4413 97.3351 68.0756 95.4372C68.71 93.5393 69.6114 91.9411 70.7466 90.6759C71.8817 89.4106 73.2172 88.445 74.7196 87.7791C76.222 87.1131 77.858 86.8135 79.5941 86.8135C81.3302 86.8135 83.0329 87.1464 84.5353 87.7791C86.0377 88.4117 87.3732 89.3773 88.5417 90.6759C89.6769 91.9411 90.5783 93.5393 91.2127 95.4372C91.847 97.3351 92.1809 99.5327 92.1809 102.063V105.126C92.1809 107.624 91.847 109.855 91.2127 111.752C90.5783 113.65 89.6769 115.249 88.5417 116.514C87.4066 117.779 86.0711 118.745 84.5687 119.377C83.0663 120.01 81.397 120.343 79.6609 120.343C77.9247 120.343 76.2554 120.01 74.753 119.377C73.2506 118.745 71.9151 117.779 70.7466 116.514C69.6114 115.249 68.71 113.65 68.0756 111.752C67.4413 109.855 67.1074 107.657 67.1074 105.126ZM72.4159 102.063V105.126C72.4159 106.891 72.5829 108.456 72.9501 109.788C73.3174 111.12 73.8182 112.252 74.4859 113.151C75.1536 114.05 75.9215 114.716 76.7896 115.182C77.6576 115.615 78.6259 115.848 79.6609 115.848C80.8628 115.848 81.8978 115.615 82.8326 115.182C83.734 114.749 84.5019 114.05 85.1029 113.151C85.7039 112.252 86.1379 111.153 86.4384 109.788C86.7389 108.456 86.8724 106.891 86.8724 105.126V102.063C86.8724 100.298 86.7055 98.7336 86.3382 97.4017C85.971 96.0699 85.4702 94.9711 84.8024 94.0388C84.1347 93.1398 83.3668 92.4406 82.4653 92.0077C81.5639 91.5749 80.5957 91.3418 79.5607 91.3418C78.5257 91.3418 77.5909 91.5749 76.7228 92.0077C75.8548 92.4406 75.0869 93.1398 74.4191 94.0388C73.7848 94.9378 73.2506 96.0699 72.9167 97.4017C72.5495 98.7336 72.3825 100.265 72.3825 102.063H72.4159Z"
              ></path>
              <path
                d="M104.367 73.8279V119.744H99.0586V73.8279H104.367ZM120.793 87.4128L109.242 102.496L102.097 110.687L101.73 104.793L106.637 98.0343L114.383 87.3795H120.76L120.793 87.4128ZM115.952 119.744L106.838 104.327L109.575 99.3995L122.129 119.744H115.952Z"
              ></path>
              <path
                d="M126.803 78.8224C126.803 77.9234 127.036 77.1576 127.537 76.5583C128.038 75.9256 128.773 75.626 129.774 75.626C130.776 75.626 131.51 75.9256 132.044 76.5583C132.545 77.1909 132.812 77.9234 132.812 78.8224C132.812 79.7214 132.545 80.4206 132.044 81.02C131.544 81.6193 130.776 81.919 129.774 81.919C128.773 81.919 128.038 81.6193 127.537 81.02C127.036 80.4206 126.803 79.6881 126.803 78.8224ZM132.378 87.4129V119.744H127.103V87.4129H132.378Z"
              ></path>
              <path
                d="M152.377 120.343C150.44 120.343 148.704 120.043 147.135 119.477C145.566 118.911 144.264 118.012 143.162 116.847C142.06 115.648 141.226 114.15 140.658 112.352C140.091 110.554 139.79 108.39 139.79 105.926V102.263C139.79 99.3995 140.124 97.0022 140.792 95.0377C141.459 93.0732 142.361 91.475 143.496 90.2763C144.631 89.0776 145.933 88.1786 147.369 87.6459C148.804 87.1131 150.273 86.8135 151.809 86.8135C153.846 86.8135 155.582 87.1464 157.018 87.8124C158.453 88.4783 159.622 89.4439 160.523 90.7424C161.425 92.041 162.092 93.6392 162.526 95.5371C162.96 97.435 163.161 99.5993 163.161 102.096V105.16H142.895V100.631H157.852V99.8989C157.786 98.2341 157.552 96.7358 157.185 95.4705C156.817 94.2053 156.216 93.2064 155.382 92.4739C154.547 91.7413 153.345 91.3751 151.809 91.3751C150.841 91.3751 149.94 91.5416 149.105 91.8745C148.27 92.2075 147.569 92.7735 146.968 93.5726C146.367 94.3717 145.933 95.5038 145.599 96.9023C145.265 98.3007 145.099 100.099 145.099 102.296V105.959C145.099 107.69 145.265 109.189 145.599 110.421C145.933 111.653 146.434 112.685 147.068 113.484C147.703 114.283 148.504 114.882 149.472 115.282C150.407 115.681 151.509 115.881 152.711 115.881C154.48 115.881 155.949 115.548 157.118 114.849C158.286 114.15 159.288 113.284 160.156 112.219L162.96 115.548C162.393 116.381 161.625 117.146 160.69 117.879C159.755 118.611 158.62 119.244 157.285 119.71C155.949 120.176 154.313 120.409 152.444 120.409L152.377 120.343Z"
              ></path>
              <path
                d="M169.538 73.8279H174.847V113.451L174.413 119.744H169.571V73.8279H169.538ZM193.042 102.13V105.16C193.042 107.724 192.809 109.921 192.341 111.852C191.874 113.75 191.173 115.348 190.271 116.614C189.336 117.879 188.235 118.811 186.899 119.444C185.564 120.076 184.061 120.376 182.359 120.376C180.656 120.376 179.22 120.043 178.018 119.344C176.783 118.678 175.781 117.679 174.947 116.414C174.112 115.149 173.478 113.584 173.01 111.786C172.543 109.988 172.209 107.923 172.042 105.659V101.63C172.209 99.3329 172.543 97.2685 173.01 95.4705C173.478 93.6392 174.112 92.1076 174.913 90.809C175.715 89.5105 176.75 88.5449 177.985 87.8789C179.187 87.213 180.656 86.8467 182.325 86.8467C183.995 86.8467 185.564 87.1464 186.899 87.7458C188.235 88.3451 189.336 89.2774 190.271 90.5093C191.173 91.7413 191.874 93.3395 192.341 95.2707C192.809 97.2019 193.042 99.4994 193.042 102.163V102.13ZM187.734 105.126V102.096C187.734 100.298 187.634 98.7335 187.4 97.4017C187.166 96.0698 186.799 94.9711 186.265 94.0721C185.731 93.2063 185.063 92.5404 184.195 92.1076C183.327 91.6747 182.258 91.4416 180.99 91.4416C179.921 91.4416 178.953 91.6747 178.152 92.1076C177.351 92.5404 176.649 93.1398 176.082 93.8723C175.514 94.6048 175.014 95.4705 174.646 96.4028C174.279 97.3351 173.979 98.334 173.812 99.3329V107.823C174.079 109.122 174.479 110.354 175.047 111.553C175.614 112.751 176.382 113.717 177.384 114.483C178.352 115.249 179.588 115.615 181.057 115.615C182.258 115.615 183.293 115.415 184.162 114.982C184.996 114.549 185.697 113.917 186.231 113.051C186.766 112.185 187.166 111.086 187.4 109.755C187.634 108.423 187.767 106.858 187.767 105.06L187.734 105.126Z"
              ></path>
              <path
                d="M198.718 105.126V102.063C198.718 99.566 199.052 97.3351 199.686 95.4372C200.32 93.5393 201.222 91.9411 202.357 90.6759C203.492 89.4106 204.828 88.445 206.33 87.7791C207.832 87.1131 209.468 86.8135 211.204 86.8135C212.941 86.8135 214.643 87.1464 216.146 87.7791C217.648 88.4117 218.984 89.3773 220.152 90.6759C221.287 91.9411 222.189 93.5393 222.823 95.4372C223.457 97.3351 223.791 99.5327 223.791 102.063V105.126C223.791 107.624 223.457 109.855 222.823 111.752C222.189 113.65 221.287 115.249 220.152 116.514C219.017 117.779 217.681 118.745 216.179 119.377C214.677 120.01 213.007 120.343 211.271 120.343C209.535 120.343 207.866 120.01 206.363 119.377C204.861 118.745 203.525 117.779 202.357 116.514C201.222 115.249 200.32 113.65 199.686 111.752C199.052 109.855 198.718 107.657 198.718 105.126ZM204.026 102.063V105.126C204.026 106.891 204.193 108.456 204.56 109.788C204.928 111.12 205.429 112.252 206.096 113.151C206.764 114.05 207.532 114.716 208.4 115.182C209.268 115.615 210.236 115.848 211.271 115.848C212.473 115.848 213.508 115.615 214.443 115.182C215.344 114.749 216.112 114.05 216.713 113.151C217.314 112.252 217.748 111.153 218.049 109.788C218.349 108.456 218.483 106.891 218.483 105.126V102.063C218.483 100.298 218.316 98.7336 217.949 97.4017C217.581 96.0699 217.081 94.9711 216.413 94.0388C215.745 93.1398 214.977 92.4406 214.076 92.0077C213.174 91.5749 212.206 91.3418 211.171 91.3418C210.136 91.3418 209.201 91.5749 208.333 92.0077C207.465 92.4406 206.697 93.1398 206.029 94.0388C205.395 94.9378 204.861 96.0699 204.527 97.4017C204.16 98.7336 203.993 100.265 203.993 102.063H204.026Z"
              ></path>
              <path
                d="M242.454 87.4129V91.6415H227.33V87.4129H242.454ZM232.271 79.5549H237.58V111.752C237.58 112.851 237.713 113.684 237.98 114.25C238.248 114.816 238.581 115.182 239.015 115.348C239.449 115.515 239.884 115.615 240.351 115.615C240.718 115.615 241.119 115.582 241.553 115.482C241.987 115.382 242.287 115.315 242.454 115.249V119.777C242.054 119.91 241.586 120.043 240.985 120.176C240.384 120.31 239.616 120.376 238.715 120.376C237.58 120.376 236.511 120.11 235.543 119.577C234.575 119.044 233.774 118.145 233.173 116.88C232.572 115.615 232.271 113.917 232.271 111.752V79.5882V79.5549Z"
              ></path></svg
          ></a>
        </div>
      </div>

      <!-- Tabs Navigation -->
      <div class="border-b border-gray-200">
        <nav class="flex">
          <button
            class="tab-button flex-1 py-3 px-4 text-center border-b-2 border-blue-500 text-blue-600 font-medium"
            data-tab="tab1"
          >
            Toestemming
          </button>
          <button
            class="tab-button flex-1 py-3 px-4 text-center border-b-2 border-transparent text-gray-500 hover:"
            data-tab="tab2"
          >
            Details
          </button>
          <button
            class="tab-button flex-1 py-3 px-4 text-center border-b-2 border-transparent text-gray-500 hover:"
            data-tab="tab3"
          >
            Over
          </button>
        </nav>
      </div>

      <!-- Tab Contents -->
      <div class="tab-contents">
        <!-- Tab 1: Overview -->
        <div class="tab-content active" id="tab1">
          <div class="p-4 sm:p-6">
            <div class="space-y-4">
              <strong>Deze website maakt gebruik van cookies</strong>
              <p>
                We gebruiken cookies om content en advertenties te
                personaliseren, om functies voor social media te bieden en om
                ons websiteverkeer te analyseren. Ook delen we informatie over
                uw gebruik van onze site met onze partners voor social media,
                adverteren en analyse. Deze partners kunnen deze gegevens
                combineren met andere informatie die u aan ze heeft verstrekt of
                die ze hebben verzameld op basis van uw gebruik van hun
                services.
              </p>
            </div>
          </div>
        </div>

        <!-- Tab 2: Cookie Details (with nested accordions) -->
        <div class="tab-content hidden" id="tab2">
          <div class="p-4 sm:p-6">
            <!-- Main Accordion Items -->
            <div class="space-y-3">
              <!-- Noodzakelijke Cookies Accordion -->
              <div
                class="accordion-item border border-gray-200 rounded-lg overflow-hidden"
              >
                <button
                  class="accordion-header w-full bg-gray-50 px-4 py-3 text-left flex justify-between items-center"
                >
                  <div class="w-full">
                    <div class="flex justify-between">
                      <span class="text-gray-800 font-bold flex items-center">
                        <svg
                          class="mr-4 accordion-icon w-5 h-5 text-gray-500 transform transition-transform duration-200"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                          <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M19 9l-7 7-7-7"
                          /></svg
                        >Noodzakelijke cookies
                        <span
                          class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                          >10</span
                        >
                      </span>
                      <label class=" cursor-pointer relative inline-flex items-center">
                        <input
                          type="checkbox"
                          id="toggle"
                          class="sr-only peer"
                          checked
                          disabled
                        />
                        <div
                          class="w-11 h-6 bg-gray-200 rounded-full peer transition-colors duration-300"
                        ></div>
                        <div
                          class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 peer-checked:translate-x-5"
                        ></div>
                      </label>
                    </div>
                    <p class="mt-4">
                      Noodzakelijke cookies helpen een website bruikbaarder te
                      maken, door basisfuncties als paginanavigatie en toegang
                      tot beveiligde gedeelten van de website mogelijk te maken.
                      Zonder deze cookies kan de website niet naar behoren
                      werken.
                    </p>
                  </div>
                </button>
                <div class="accordion-content">
                  <div class="p-4 bg-white">
                    <!-- Sub-accordion for specific cookies -->
                    <div class="mt-3 space-y-2">
                      <details
                        class="group border border-gray-100 rounded shadow-xs bg-gray-50"
                      >
                        <summary
                          class="cursor-pointer w-full px-3 py-2 text-left flex flex-col"
                        >
                          <div class="flex justify-between items-center">
                            <span
                              class="text-gray-800 font-bold flex items-center"
                            >
                              Cookiebot
                              <span
                                class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                                >1</span
                              >
                            </span>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke-width="1.5"
                              stroke="currentColor"
                              class="block h-4 w-4 transition-all duration-300 group-open:rotate-180 text-gray-400"
                            >
                              <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M19.5 8.25l-7.5 7.5-7.5-7.5"
                              ></path>
                            </svg>
                          </div>
                          <a
                            class="font-bold"
                            href="https://www.cookiebot.com/goto/privacy-policy/"
                            target="_blank"
                            rel="noopener noreferrer"
                            >Meer informatie over deze aanbieder &#10138;</a
                          >
                        </summary>

                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">CookieConsent</strong>
                          <p class="p-3">
                            Slaat de cookiestatus van de gebruiker op voor het
                            huidige domein
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 jaar
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                      </details>
                      <details
                        class="group border border-gray-100 rounded shadow-xs bg-gray-50"
                      >
                        <summary
                          class="cursor-pointer w-full px-3 py-2 text-left flex flex-col"
                        >
                          <div class="flex justify-between items-center">
                            <span
                              class="text-gray-800 font-bold flex items-center"
                            >
                              Feedback Company
                              <span
                                class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                                >1</span
                              >
                            </span>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke-width="1.5"
                              stroke="currentColor"
                              class="block h-4 w-4 transition-all duration-300 group-open:rotate-180 text-gray-400"
                            >
                              <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M19.5 8.25l-7.5 7.5-7.5-7.5"
                              ></path>
                            </svg>
                          </div>
                          <a
                            class="font-bold"
                            href="https://www.feedbackcompany.com/privacy-statement/"
                            target="_blank"
                            rel="noopener noreferrer"
                            >Meer informatie over deze aanbieder &#10138;</a
                          >
                        </summary>

                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">XSRF-TOKEN </strong>
                          <p class="p-3">
                            Zorgt voor browsingveiligheid, door vervalsing van
                            cross-site verzoeken te voorkomen. Deze cookie is
                            essentieel voor de veiligheid van de website en
                            bezoeker.
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 dag
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                      </details>
                      <details
                        class="group border border-gray-100 rounded shadow-xs bg-gray-50"
                      >
                        <summary
                          class="cursor-pointer w-full px-3 py-2 text-left flex flex-col"
                        >
                          <div class="flex justify-between items-center">
                            <span
                              class="text-gray-800 font-bold flex items-center"
                            >
                              Google
                              <span
                                class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                                >1</span
                              >
                            </span>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke-width="1.5"
                              stroke="currentColor"
                              class="block h-4 w-4 transition-all duration-300 group-open:rotate-180 text-gray-400"
                            >
                              <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M19.5 8.25l-7.5 7.5-7.5-7.5"
                              ></path>
                            </svg>
                          </div>
                          <a
                            class="font-bold"
                            href="https://business.safety.google/privacy/"
                            target="_blank"
                            rel="noopener noreferrer"
                            >Meer informatie over deze aanbieder &#10138;</a
                          >
                        </summary>
                        <p class="px-3 py-2 text-left">
                          Sommige gegevens die door deze provider worden
                          verzameld, zijn bedoeld voor personalisatie en het
                          meten van de effectiviteit van advertenties.
                        </p>
                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">test_cookie</strong>
                          <p class="p-3">
                            Gebruikt om te controleren of de browser van de
                            gebruiker cookies ondersteunt.
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 dag
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                      </details>
                      <details
                        class="group border border-gray-100 rounded shadow-xs bg-gray-50"
                        open=""
                      >
                        <summary
                          class="cursor-pointer w-full px-3 py-2 text-left flex flex-col"
                        >
                          <div class="flex justify-between items-center">
                            <span
                              class="text-gray-800 font-bold flex flex-col"
                            >
                                <p class="font-bold">platform.getqonfi.com</p>
                                <p class="font-bold">sitesearch360.com</p>
                                <p class="font-bold">webshopapp.com</p>
                                <p class="font-bold">www.dekbedovertrekkenenzo.nl</p>
                              <span
                                class=" flex place-items-center font-bold text-xs bg-gray-300 w-4 h-4 rounded-full ml-2 px-1"
                                >4</span
                              >
                            </span>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke-width="1.5"
                              stroke="currentColor"
                              class="block h-4 w-4 transition-all duration-300 group-open:rotate-180 text-gray-400"
                            >
                              <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M19.5 8.25l-7.5 7.5-7.5-7.5"
                              ></path>
                            </svg>
                          </div>
                        </summary>

                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">__cf_bm [x4]</strong>
                          <p class="p-3">
                            Deze cookie wordt gebruikt om onderscheid te maken tussen mensen en bots. Dit is gunstig voor de website om juiste rapporten over het gebruik van de website te maken.

                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 dag
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                        <br />
                        
                      </details>
                      <details class="group border border-gray-100 rounded shadow-xs bg-gray-50" open="">
                        <summary class="cursor-pointer w-full px-3 py-2 text-left flex flex-col">
                          <div class="flex justify-between items-center">
                            <span class="text-gray-800 font-bold flex items-center">
                              www.dekbedovertrekkenenzo.nl
                              <span class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1">3</span>
                            </span>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="block h-4 w-4 transition-all duration-300 group-open:rotate-180 text-gray-400">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5"></path>
                            </svg>
                          </div>
                        </summary>

                        <div class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300">
                          <strong class="p-3">COOKIELAW </strong>
                          <p class="p-3">
                            Bepaalt of de bezoeker de cookies heeft
                            geaccepteerd.
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 jaar
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                        <br>
                        <div class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300">
                          <strong class="p-3">COOKIELAW_STATS</strong>
                          <p class="p-3">
                            Slaat de cookiestatus van de gebruiker op voor het huidige domein

                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 jaar
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                      </details>
                    </div>
                    <!-- Sub-accordion for specific cookies ends-->
                  </div>
                </div>
              </div>
              <!-- Noodzakelijke Cookies Accordion ends-->
              <!-- Voorkeuren Cookies Accordion -->
              <div
                class="accordion-item border border-gray-200 rounded-lg overflow-hidden"
              >
                <button
                  class="accordion-header w-full bg-gray-50 px-4 py-3 text-left flex justify-between items-center"
                >
                  <div class="w-full">
                    <div class="flex justify-between">
                      <span class="text-gray-800 font-bold flex items-center">
                        <svg
                          class="mr-4 accordion-icon w-5 h-5 text-gray-500 transform transition-transform duration-200"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                          <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M19 9l-7 7-7-7"
                          /></svg
                        >Voorkeuren
                        <span
                          class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                          >4</span
                        >
                      </span>
                        <label class=" cursor-pointer relative inline-flex items-center">
                        <input
                          type="checkbox"
                          id="toggle"
                          class="sr-only peer"
                          checked
                          
                        />
                        <div
                          class="w-11 h-6 bg-gray-200 peer-checked:bg-green-300 rounded-full peer transition-colors duration-300"
                        ></div>
                        <div
                          class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 peer-checked:translate-x-5"
                        ></div>
                        </label>
                    </div>
                    <p class="mt-4">
                      Voorkeurscookies zorgen ervoor dat een website informatie
                      kan onthouden die van invloed is op het gedrag en de
                      vormgeving van de website, zoals de taal van uw voorkeur
                      of de regio waar u woont.
                    </p>
                  </div>
                </button>
                <div class="accordion-content">
                  <div class="p-4 bg-white">
                    <!-- Sub-accordion for specific cookies -->
                    <div class="mt-3 space-y-2">
                      <details
                        class="group border border-gray-100 rounded shadow-xs bg-gray-50"
                      >
                        <summary
                          class="cursor-pointer w-full px-3 py-2 text-left flex flex-col"
                        >
                          <div class="flex justify-between items-center">
                            <span
                              class="text-gray-800 font-bold flex items-center"
                            >
                              Site Search 360
                              <span
                                class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                                >2</span
                              >
                            </span>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke-width="1.5"
                              stroke="currentColor"
                              class="block h-4 w-4 transition-all duration-300 group-open:rotate-180 text-gray-400"
                            >
                              <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M19.5 8.25l-7.5 7.5-7.5-7.5"
                              />
                            </svg>
                          </div>
                          <a
                            class="font-bold"
                            href="https://www.sitesearch360.com/privacy-policy"
                            target="_blank"
                            rel="noopener noreferrer"
                            >Meer informatie over deze aanbieder &#10138;</a
                          >
                        </summary>

                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">ssi--lastInteraction </strong>
                          <p class="p-3">
                            Noodzakelijk om de zoekbalkfunctie van de website te
                            optimaliseren. De cookie zorgt voor nauwkeurige en
                            snelle zoekresultaten.
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 dag
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                      </details>
                      <details
                        class="group border border-gray-100 rounded shadow-xs bg-gray-50"
                      >
                        <summary
                          class="cursor-pointer w-full px-3 py-2 text-left flex flex-col"
                        >
                          <div class="flex justify-between items-center">
                            <span
                              class="text-gray-800 font-bold flex items-center"
                            >
                              www.dekbedovertrekkenenzo.nl
                              <span
                                class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                                >2</span
                              >
                            </span>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke-width="1.5"
                              stroke="currentColor"
                              class="block h-4 w-4 transition-all duration-300 group-open:rotate-180 text-gray-400"
                            >
                              <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M19.5 8.25l-7.5 7.5-7.5-7.5"
                              />
                            </svg>
                          </div>
                        </summary>

                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">COOKIELAW_ADS </strong>
                          <p class="p-3">
                            Bepaalt of de bezoeker de cookies heeft
                            geaccepteerd.
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 jaar
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                        <br />
                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">COOKIELAW_SOCIAL </strong>
                          <p class="p-3">
                            Bepaalt of de bezoeker de cookies heeft
                            geaccepteerd.
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 jaar
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                      </details>
                    </div>
                    <!-- Sub-accordion for specific cookies ends-->
                  </div>
                </div>
              </div>
              <!-- Voorkeuren Cookies Accordion ends-->
              <!-- Statistieken Cookies Accordion starts-->
              <div
                class="accordion-item border border-gray-200 rounded-lg overflow-hidden"
              >
                <button
                  class="accordion-header w-full bg-gray-50 px-4 py-3 text-left flex justify-between items-center"
                >
                  <div class="w-full">
                    <div class="flex justify-between">
                      <span class="text-gray-800 font-bold flex items-center">
                        <svg
                          class="mr-4 accordion-icon w-5 h-5 text-gray-500 transform transition-transform duration-200 rotate-180"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                          <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M19 9l-7 7-7-7"
                          ></path>
                        </svg>
                        Statieken
                        <span
                          class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                          >0</span
                        >
                      </span>
                        <label class=" cursor-pointer relative inline-flex items-center">
                        <input
                          type="checkbox"
                          id="toggle"
                          class="sr-only peer"
                          checked
                          
                        />
                        <div
                          class="w-11 h-6 bg-gray-200 peer-checked:bg-green-300 rounded-full peer transition-colors duration-300"
                        ></div>
                        <div
                          class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 peer-checked:translate-x-5"
                        ></div>
                        </label>
                    </div>
                    <p class="mt-4">
                      Statistische cookies helpen eigenaren van websites
                      begrijpen hoe bezoekers hun website gebruiken, door
                      anoniem gegevens te verzamelen en te rapporteren.
                    </p>
                  </div>
                </button>
                <div class="accordion-content open">
                  <div class="bg-white">
                    <!-- Sub-accordion for specific cookies -->
                    <div class="mt-3">
                      <details
                        class="group border border-gray-100 rounded shadow-xs bg-gray-50"
                        open=""
                      >
                        <summary
                          class="cursor-pointer w-full px-3 py-2 text-left flex flex-col"
                        >
                          <div class="flex justify-between items-center">
                            <span
                              class="text-gray-800 font-bold flex items-center"
                            >
                              Wij gebruiken geen cookies van dit type
                            </span>
                          </div>
                        </summary>
                      </details>
                    </div>
                    <!-- Sub-accordion for specific cookies ends-->
                  </div>
                </div>
              </div>
              <!-- Statistieken Cookies Accordion ends-->
              <!-- Marketing Cookies Accordion -->
              <div
                class="accordion-item border border-gray-200 rounded-lg overflow-hidden"
              >
                <button
                  class="accordion-header w-full bg-gray-50 px-4 py-3 text-left flex justify-between items-center"
                >
                  <div class="w-full">
                    <div class="flex justify-between">
                      <span class="text-gray-800 font-bold flex items-center">
                        <svg
                          class="mr-4 accordion-icon w-5 h-5 text-gray-500 transform transition-transform duration-200"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                          <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M19 9l-7 7-7-7"
                          /></svg
                        >Marketing
                        <span
                          class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                          >33</span
                        >
                      </span>
                        <label class=" cursor-pointer relative inline-flex items-center">
                        <input
                          type="checkbox"
                          id="toggle"
                          class="sr-only peer"
                          checked
                          
                        />
                        <div
                          class="w-11 h-6 bg-gray-200 peer-checked:bg-green-300 rounded-full peer transition-colors duration-300"
                        ></div>
                        <div
                          class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 peer-checked:translate-x-5"
                        ></div>
                        </label>
                    </div>
                    <p class="mt-4">
                      Marketingcookies worden gebruikt om bezoekers te volgen wanneer ze verschillende websites bezoeken. Hun doel is advertenties weergeven die zijn toegesneden op en relevant zijn voor de individuele gebruiker. Deze advertenties worden zo waardevoller voor uitgevers en externe adverteerders.
                    </p>
                  </div>
                </button>
                <div class="accordion-content">
                  <div class="p-4 bg-white">
                    <!-- Sub-accordion for specific cookies -->
                    <div class="mt-3 space-y-2">
                      <details
                        class="group border border-gray-100 rounded shadow-xs bg-gray-50"
                      >
                        <summary
                          class="cursor-pointer w-full px-3 py-2 text-left flex flex-col"
                        >
                          <div class="flex justify-between items-center">
                            <span
                              class="text-gray-800 font-bold flex items-center"
                            >
                              Meta Platforms, Inc.
                              <span
                                class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                                >3</span
                              >
                            </span>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke-width="1.5"
                              stroke="currentColor"
                              class="block h-4 w-4 transition-all duration-300 group-open:rotate-180 text-gray-400"
                            >
                              <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M19.5 8.25l-7.5 7.5-7.5-7.5"
                              />
                            </svg>
                          </div>
                          <a
                            class="font-bold"
                            href="https://www.facebook.com/policy.php/"
                            target="_blank"
                            rel="noopener noreferrer"
                            >Meer informatie over deze aanbieder &#10138;</a
                          >
                        </summary>

                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">ssi--lastInteraction </strong>
                          <p class="p-3">
                            Noodzakelijk om de zoekbalkfunctie van de website te
                            optimaliseren. De cookie zorgt voor nauwkeurige en
                            snelle zoekresultaten.
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 dag
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                      </details>
                      <details
                        class="group border border-gray-100 rounded shadow-xs bg-gray-50"
                      >
                        <summary
                          class="cursor-pointer w-full px-3 py-2 text-left flex flex-col"
                        >
                          <div class="flex justify-between items-center">
                            <span
                              class="text-gray-800 font-bold flex items-center"
                            >
                              www.dekbedovertrekkenenzo.nl
                              <span
                                class="font-bold text-xs bg-gray-300 rounded-full ml-2 px-1"
                                >2</span
                              >
                            </span>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke-width="1.5"
                              stroke="currentColor"
                              class="block h-4 w-4 transition-all duration-300 group-open:rotate-180 text-gray-400"
                            >
                              <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M19.5 8.25l-7.5 7.5-7.5-7.5"
                              />
                            </svg>
                          </div>
                        </summary>

                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">COOKIELAW_ADS </strong>
                          <p class="p-3">
                            Bepaalt of de bezoeker de cookies heeft
                            geaccepteerd.
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 jaar
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                        <br />
                        <div
                          class="flex flex-col p-3 bg-gray-200 divide-y divide-gray-300"
                        >
                          <strong class="p-3">COOKIELAW_SOCIAL </strong>
                          <p class="p-3">
                            Bepaalt of de bezoeker de cookies heeft
                            geaccepteerd.
                          </p>
                          <p class="p-3">
                            <strong>Maximale bewaartermijn:</strong> 1 jaar
                          </p>
                          <p class="p-3"><strong>Type:</strong> HTTP-cookie</p>
                        </div>
                      </details>
                    </div>
                    <!-- Sub-accordion for specific cookies ends-->
                  </div>
                </div>
              </div>
              <!-- Marketing Cookies Accordion ends-->
            </div>
          </div>
        </div>

        <!-- Tab 3: Privacy Policy -->
        <div class="tab-content hidden" id="tab3">
          <div class="p-4 sm:p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-3">
              Privacy beleid
            </h3>
            <div class="text-sm text-gray-600 space-y-2">
              <p>
                Cookies zijn kleine tekstbestanden die door websites kunnen
                worden gebruikt om gebruikerservaringen efficiënter te maken.
              </p>
              <p>
                Volgens de wet mogen wij cookies op uw apparaat opslaan als ze
                strikt noodzakelijk zijn voor het gebruik van de site. Voor alle
                andere soorten cookies hebben we uw toestemming nodig.
              </p>
              <p>
                Deze website maakt gebruik van verschillende soorten cookies.
                Sommige cookies worden geplaatst door diensten van derden die op
                onze pagina's worden weergegeven.
              </p>
              <p>
                Via de cookieverklaring op onze website kunt u uw toestemming op
                elk moment wijzigen of intrekken.
              </p>
            </div>
          </div>
        </div>
      </div>

      <!-- Banner Footer -->
      <div class="p-4 sm:p-6 border-t border-gray-200 bg-gray-50">
        <div class="flex flex-col sm:flex-row justify-between gap-3">
          <button
            class="order-2 sm:order-1 px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded font-medium transition-colors rounded-full"
            id="decline-all"
          >
            Weigeren
          </button>
          <div class="order-1 sm:order-2 flex gap-2">
            <button
              class="px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded font-medium transition-colors rounded-full"
              id="accept-selected"
            >
              Selectie accepteren
            </button>
            <button
              class="px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded font-medium transition-colors rounded-full"
              id="accept-all"
            >
              Alles accepteren
            </button>
          </div>
        </div>
      </div>
    </div>

    <script>
      // Tab switching functionality
      document.querySelectorAll('.tab-button').forEach((button) => {
        button.addEventListener('click', () => {
          // Remove active state from all tabs
          document.querySelectorAll('.tab-button').forEach((btn) => {
            btn.classList.remove('border-blue-500', 'text-blue-600')
            btn.classList.add('border-transparent', 'text-gray-500')
          })

          // Add active state to clicked tab
          button.classList.remove('border-transparent', 'text-gray-500')
          button.classList.add('border-blue-500', 'text-blue-600')

          // Hide all tab contents
          document.querySelectorAll('.tab-content').forEach((content) => {
            content.classList.add('hidden')
          })

          // Show the corresponding tab content
          const tabId = button.getAttribute('data-tab')
          document.getElementById(tabId).classList.remove('hidden')
        })
      })

      // Main accordions functionality
      document.querySelectorAll('.accordion-header').forEach((header) => {
        header.addEventListener('click', () => {
          const content = header.nextElementSibling
          const icon = header.querySelector('.accordion-icon')

          content.classList.toggle('open')
          icon.classList.toggle('rotate-180')
        })
      })

      // Sub-accordions functionality
      document.querySelectorAll('.sub-accordion-header').forEach((header) => {
        header.addEventListener('click', () => {
          const content = header.nextElementSibling
          const icon = header.querySelector('.sub-accordion-icon')

          content.classList.toggle('open')
          icon.classList.toggle('rotate-180')
        })
      })

      // Close banner
      document.getElementById('close-banner').addEventListener('click', () => {
        document.querySelector('.cookiebot-banner').style.display = 'none'
      })

      // Cookie consent buttons
      document.getElementById('accept-all').addEventListener('click', () => {
        document
          .querySelectorAll('input[type="checkbox"]')
          .forEach((checkbox) => {
            checkbox.checked = true
          })
        setTimeout(() => {
          document.querySelector('.cookiebot-banner').style.display = 'none'
        }, 500)
      })

      document.getElementById('decline-all').addEventListener('click', () => {
        document
          .querySelectorAll('input[type="checkbox"]:not([])')
          .forEach((checkbox) => {
            checkbox.checked = false
          })
        setTimeout(() => {
          document.querySelector('.cookiebot-banner').style.display = 'none'
        }, 500)
      })

      document
        .getElementById('accept-selected')
        .addEventListener('click', () => {
          setTimeout(() => {
            document.querySelector('.cookiebot-banner').style.display = 'none'
          }, 500)
        })

      const toggle = document.getElementById('toggle')
      toggle.addEventListener('change', () => {
        console.log('Toggle is ' + (toggle.checked ? 'ON' : 'OFF'))
      })
    </script>
  </body>
</html>
